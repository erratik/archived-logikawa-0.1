	...
	self: {
		...
		writeStory: function(params, cb) {
			...
			if (!!params.from && !!params.to) {
				...
				fromTs =
					Number(moment(params.day, 'YYYYMMDD').startOf('day').format('x')) ||
					Number(moment().startOf('day').format('x'));
				toTs =
					Number(moment(params.day, 'YYYYMMDD').endOf('day').format('x')) ||
					Number(moment().endOf('day').format('x'));
			}
				...
				Rain.findAllRain(
					...
					(rain) => {
						...
						if (!drops) {
							...
							drops.items = drops.items.map((drop) =>
								DropService.enrichDrop(
									drop,
									dimensions[drop.space].filter((dim) => dim.type === drop.type)
								)
							);

							[ stories ] = docs
								.filter((o) => !!o._id)
								.map((story) => story.items.filter((item) => item.type === 'rain.storyline'));
										...
										segments = segments.map((segment) => {
											if (!!segment.activities) {
												segment.activities = segment.activities.map((activity) => {
													activity.startTime = activity.timestamp = Number(
														moment(activity.startTime).format('x')
													);
													activity.endTime = Number(moment(activity.endTime).format('x'));
													activity.drops = !!drops.items
														? drops.items.filter((d, i) => {
																const isActivityDrop =
																	d.timestamp > activity.startTime &&
																	d.timestamp < activity.endTime;
																if (isActivityDrop) {
											...
											segment.startTime = segment.timestamp = Number(
												moment(segment.startTime).format('x')
											);
											segment.endTime = Number(moment(segment.endTime).format('x'));

											if (segment.type === 'place') {
												const placeDrops = drops.items.filter(
													(item) =>
														item.timestamp > segment.startTime &&
														item.timestamp < segment.endTime
												);
												segment.place.drops = drops.items.length ? drops.items : null;

												// if this is a place, let's put the activities that need to be in it
												const placeActivites = segment.activities.filter(
													(item) =>
														item.timestamp > segment.startTime &&
														item.timestamp < segment.endTime
												);
			...
			var addSchema = function(callback) {
				drops = drops.map((drop, i) => {
					...
					Object.keys(drop.content).map((key) => {
						if (possibleTimestampKeys.includes(key)) {
							drop.timestamp = dateFormat
								? moment(drop.content[key], dateFormat).format('x')
								: moment.unix(drop.content[key]).format('X');
							drop.timestamp =
								key !== 'timestamp'
									? Number(drop.timestamp)
									: Number(moment(drop.content[key], 'X').format('x'));
						}
					});
				...
				if (drops.length) {
					that.findOneAndUpdate(
						...
						(err, updated) => {
							if (!!updated) {
								const lastDrop = _.max(
									_.filter(updated.drops, {
										type
									}),
									function(o) {
										return o.timestamp;
			...
			that.findOne(query, function(err, spaceDrops) {
				if (!!spaceDrops) {
					existingDropTimestamps = spaceDrops.drops.map((drop) => {
						if (drop.timestamp && drop.type === type) return drop.timestamp;
					});
				}
